...
export function renderFirstRowTable(app) {
    ...
    let tableRow = "<tr data-row-type='first-row' data-row-index=1 class='crud-group'>";
    ...
    dataFirstRow.forEach((cell) => {
        tableRow += `<td type="cell" data-row-type="first-row" data-row-index=1 data-column-index=${
            ...
            cell.index === 0 || cell.index === dataFirstRow.length - 1 ? 'cell' : 'column-data cell'
        } " style="background-color:${cell.bg || 'color'}" >${cell.data ? cell.data : buttonEdit}</td>`;
    });
    tableRow += '</tr>';
