...
function saveLocalStorage(JsonData) {
    ...
    console.log(`%c ${getCurrentDataTime()} - save local successed`, 'background: blue; color: white');
}
...
export function cleanupLocalStorage() {
    ...
    console.log(`%c ${getCurrentDataTime()} - cleaning successed`, 'background: red; color: white');
}
...
export function readData(app) {
    ...
    if (data.length === 0) {
        console.log(`%c ${getCurrentDataTime()} - read data from default file`, 'background: green; color: white');

        return readTextFile(`./DB/${app.document}.json`);
    }

    console.log(`%c ${getCurrentDataTime()} - read data from local store`, 'background: purple; color: white');
