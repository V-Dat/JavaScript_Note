...
export function toggleStackBlitz() {
    ...
    if (modalStackBlitz.style.display === 'none') {
        ...
        showModal(modalStackBlitz, buttonStackBlitz, './Assets/Icons/back-icon.svg');
    } else {
        buttonOpenJSPlaygrounds.style.display = 'block';
        enableGitLabel();
        toggleArrowBlock(true);
        hideModal(modalStackBlitz, buttonStackBlitz, './Assets/Icons/thunder-active-icon.svg');
    }
}
...
export function limitHeightAndWidthBody() {
    $('body').style.cssText = `width: 100vw ; height: 100vh; overflow:hidden;margin: 0px`;
}
export function unlimitHeightAndWidthBody() {
    $('body').style.cssText = `width: auto ; height: auto; overflow:unset; padding: 0 78px`;
}
...
export function toggleJSPlaygrounds() {
    ...
    if (modalJSPlaygrounds.style.display === 'none') {
        ...
        disableGitLabel();
        toggleArrowBlock(false);
        showModal(modalJSPlaygrounds, buttonOpenJSPlaygrounds, './Assets/Icons/back-icon.svg');
    } else {
        buttonStackBlitz.style.display = 'block';
        enableGitLabel();
        toggleArrowBlock(true);
        hideModal(modalJSPlaygrounds, buttonOpenJSPlaygrounds, './Assets/Icons/Js-icon.svg');
    }
}
