...
import { embedProject, toggleJSPlaygrounds, toggleStackBlitz } from './ModalPlaygrounds.js';
import { arrowFeature, handleClickArrowFeature } from './ArrowFeature.js';
import { hideModalRowDetail } from './ModalRowDetail.js';
import { onChangeHeadingContent, onChangeHeadingMethod } from './RenderHeading.js';
...
export function processListener() {
    ...
    inputFile.addEventListener('change', (event) => renderDataImport(event, _this));
    buttonEditmainContent.addEventListener('click', () => handleEditMainContent(_this.JsonData));
    ...
    buttonCloseModalRowDetail.addEventListener('click', () => hideModalRowDetail(_this));
    buttonEditHeadingContent.addEventListener('click', () => onChangeHeadingContent(_this.JsonData));
    buttonEditHeadingMethod.addEventListener('click', () => onChangeHeadingMethod(_this.JsonData));
    buttonEditReferance.addEventListener('click', () => onChangeReferanceContent(_this.JsonData));
    butthandleClickEditNote.addEventListener('click', () => handleClickEditNote(_this));
    actionsNote.addEventListener('click', (event) => handleActionNote(event, _this));
    buttonSwitchDoc.addEventListener('click', () => handleClickMenuBar(_this));
    radioDocument.addEventListener('click', (event) => handleRadioDocument(event, _this));
